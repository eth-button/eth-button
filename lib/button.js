...
export default function createButton(document, scriptEl, rpc) {
  ...
  const blockie = createIcon(config.account, 8, 8);
  blockie.className = "EthDonateButton--Blockie";
  blockie.title = config.account;
  container.appendChild(blockie);
  ...
  addressLink.href = "https://etherscan.io/address/" + config.account + "/";
  addressLink.innerHTML = config.account;
  addressLink.title = config.account;
  ...
  button.addEventListener("click", function clickHandler(e) {
    if (buttonActive) {
      ...
      sendEth(rpc, config.account, inputElement.value)
        .then(function(res) {
          doneButton();
          console.log(res);
        })
